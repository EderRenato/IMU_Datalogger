...
void display_menu_page(int page) {
    ...
    switch(page) {
        case 0:
            ssd1306_draw_string(&ssd, "Menu Principal", 1, 5);
            ssd1306_draw_string(&ssd, "1. Status SD", 1, 20);
            ssd1306_draw_string(&ssd, "2. Dados IMU", 1, 35);
            ssd1306_draw_string(&ssd, "3. Config", 1, 50);
            break;
            
        case 1:
            ssd1306_draw_string(&ssd, "Status SD Card", 1, 5);
            ssd1306_draw_string(&ssd, sd_mounted ? "Montado" : "Desmontado", 1, 20);
            ssd1306_draw_string(&ssd, "Arquivos:", 1, 35);
            ssd1306_draw_string(&ssd, filename, 1, 50);
            break;
            
        case 2:
            ssd1306_draw_string(&ssd, "Dados IMU", 1, 5);
            char accel_str[30];
            sprintf(accel_str, "A: %.2f,%.2f, %.2f", accel[0], accel[1], accel[2]);
            ssd1306_draw_string(&ssd, accel_str, 1, 15);
            
            char gyro_str[30];
            sprintf(gyro_str, "G: %.2f,%.2f,%.2f", gyro[0], gyro[1], gyro[2]);
            ssd1306_draw_string(&ssd, gyro_str, 1, 35);
            break;
...
void capture_imu_data_and_save() {
    ...
    const int total_amostras = 10000;
    const int intervalo_ms = 1;
...
int main()
{
    ...
    stdio_flush(); 
    run_help();
    ssd1306_draw_string(&ssd, "Sistema Iniciado", 1, 32);
    display_menu_page(0);
    while (true) {
        ...
        switch (cRxedChar) {
            case 'a': // Monta o SD card se pressionar 'a'
                ssd1306_fill(&ssd, false);
                ssd1306_draw_string(&ssd, "Montando SD", 1, 5);
                printf("\nMontando o SD...\n");
                run_mount();
                ssd1306_draw_string(&ssd, "SD Montado", 1, 20);
                printf("\nSd Montado");
            ...
            case 'b': // Desmonta o SD card se pressionar 'b'
                ssd1306_fill(&ssd, false);
                ssd1306_draw_string(&ssd, "Desmontando SD", 1, 5);
                printf("\nDesmontando o SD. Aguarde...\n");
                run_unmount();
                gpio_put(RED_LED, false); gpio_put(GREEN_LED, false);
                ssd1306_draw_string(&ssd, "SD desmontado", 1, 20);
                printf("\nEscolha o comando (h = help):  ");
                break;
            ...
            case 'f': // Captura dados do IMU e salva no arquivo se pressionar 'f'
                ssd1306_fill(&ssd, false);
                ssd1306_draw_string(&ssd, "Gravando Dados", 1, 5);
                set_buzzer_tone(BUZZER_A, D4);
                sleep_ms(1000);
                stop_buzzer(BUZZER_A);
                capture_imu_data_and_save();
                ssd1306_draw_string(&ssd, "Dados Salvos", 1, 20);
            ...
            case 'g': // Formata o SD card se pressionar 'g'
                ssd1306_fill(&ssd, false);
                ssd1306_draw_string(&ssd, "Formatando SD", 1, 5);
                printf("\nProcesso de formatação do SD iniciado. Aguarde...\n");
                run_format();
                ssd1306_draw_string(&ssd, "SD formatado", 1, 20);
